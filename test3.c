...
int remove_last_node(struct LIST**);
...
int main() {
    ...
    scanf("%d", &toRemove);
    if (l->last->data == toRemove) {
        remove_last_node(&l);
    }
    remove_all_nodes_by_key(&l, toRemove);
    print_list(&l);
    printf("%d",l->last->data);
    free_list(l);
    l = NULL;
    return 0;
}
...
int remove_last_node(struct LIST** l) {
    if ((*l)->count == 0) {
        return 0;
    }
    struct NODE* p1 = (*l)->last->prev;
    struct NODE* p2 = (*l)->last;
    (*l)->last = p1;
    (*p1).next = NULL;
    free(p2);
    (*l)->count -= 1;
